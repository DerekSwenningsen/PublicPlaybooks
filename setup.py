...
omsagent_default_incoming_port = "25226" ## 25324 for Bluestem
...
def install_omsagent(workspace_id, primary_key, oms_agent_install_url):
    ...
    if omsagent_proxy_conf is not None:
        ...
        command_tokens = ["sh", omsagent_file_name, "-w", workspace_id, "-s", primary_key, "-p", omsagent_proxy_conf]
    else:
        command_tokens = ["sh", omsagent_file_name, "-w", workspace_id, "-s", primary_key]
...
def main():
    ...
    if install_omsagent(workspace_id=workspace_id, primary_key=primary_key, oms_agent_install_url=oms_agent_install_url):
